...
program.option(
  "-d, --debug [port]",
  "Enable debug mode (deprecated) with optional port"
);
...
program.option(
  "-o, --only [globs]",
  "Matching files will *only* be transpiled",
  arrayify,
  null
);
program.option(
  "-i, --ignore [globs]",
  'Matching files will not be transpiled, but will still be watched. Default value is "node_modules". If you specify this option and still want to exclude modules, be sure to add it to the list.',
  arrayify,
  ["node_modules"]
);
program.option(
...
program.option(
  "-L, --use-polling",
  'In some filesystems watch events may not work correcly. This option enables "polling" which should mitigate this type of issue'
);
program.option(
  "-D, --disable-autowatch",
...
program.option(
  "-m, --message [string]",
  "Set custom message displayed on restart",
  ">>> Restarting due to change in file(s): %s"
);
program.option("-c, --config-file [string]", "Babel config file path");
program.option(
  "--root-mode [mode]",
  "The project-root resolution mode. One of 'root' (the default), 'upward', or 'upward-optional'. See https://babeljs.io/docs/en/options#rootmode"
);
program.option(
  "--clear-console",
  "If set, will clear console on each restart. Restart message will not be shown"
);
program.option(
  "--before-restart <command>",
  'Set a custom command to be run before each restart, for example "npm run lint"'
);
program.option(
  "--restart-timeout <ms>",
  "Set the maximum time to wait before forcing a restart. Useful if your app does graceful cleanup.",
  Number,
  2000
);
...
program.option(
  ...
  "Set a string to issue a manual restart. Set to `false` to pass stdin directly to process.",
  booleanify,
  "rs"
);
program.option(
...
if (!mainModule) {
  logError(
    "Main script not specified. If you are using `--inspect` or similar options, please add a `--` like so:"
  );
...
function handleChange(file) {
  ...
  debugWatcher(
    "Change detected in file: %s. File used by program (%s). File ignored (%s).",
    file,
    isUsed,
    isIgnored
  );
...
async function handleFileLoad(filename) {
  ...
  try {
    ...
    if (err instanceof IgnoredFileError) {
      ...
      debugCompile(
        "File %s ignored due to extension or intentional ignore rule.",
        filename
      );
    } else {
